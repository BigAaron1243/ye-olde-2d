...
class Circle {
    public:
        //radius, x-coordinate, y-coordinate, x-velocity, y-velocity, mass, coefficient of friction (mu)
        float r, x, y, vx, vy, m, u;
        void set_values(float,float,float,int);
        std::vector<std::vector<float>> edge;

};

void Circle::set_values (float setr, float setx, float sety, int setresolution) {
    r = setr; x = setx; y = sety;
